...
class CogStatTestCase(unittest.TestCase):
    ...
    def test_explore_variable_pairs(self):
        ...
        self.assertTrue("Pearson's correlation: <i>r</i>(28) = -0.141, 95% CI [-0.477, 0.231], <i>p</i> = 0.456" in result[5])
        self.assertTrue('y = -21.811x + 300.505' in result[3])
        self.assertTrue("Spearman's rank-order correlation: <i>r<sub>s</sub></i>(28) = -0.363, 95% CI [-0.640, -0.003], <i>p</i> = 0.048" in result[5])

        # Ord variables
        data.data_measlevs['a'] = 'ord'
        data.data_measlevs['b'] = 'ord'
        result = data.explore_variable_pair('a', 'b')
        self.assertTrue("Spearman's rank-order correlation: <i>r<sub>s</sub></i>(28) = -0.363, 95% CI [-0.640, -0.003], <i>p</i> = 0.048" in result[4])
        data.data_measlevs['a'] = 'int'
        ...
        # Chi-square
        self.assertTrue('(4, <i>N</i> = 30) = 8.312' in result[4])
        self.assertTrue('<i>p</i> = 0.081' in result[4])
